...
const MessageItem = ({
  ...
  const audioDuration = message.mediaSize
      ? Math.round(message.mediaSize / 10000)
      : 30; // Estimate duration from file size
  ...
  const fileName = message.mediaName || message.content ||
      (message.contentType === 'document' ? 'Document' :
          message.contentType === 'video' ? 'Video' :
              message.contentType === 'audio' ? 'Audio' : 'File');


  const mediaUrl = `${API_BASE_URL_FOR_MEDIA}/${message.mediaUrl}`;
  ...
  const getMediaUrl = mediaPath => {
    ...
    const formattedPath = mediaPath.startsWith('/')
        ? mediaPath
        : `/${mediaPath}`;
...
// Render document message
  ...
  const renderVideoMessage = () => {
    ...
    const videoName = message.mediaName ||
        (message.mediaUrl ? message.mediaUrl.split('/').pop() : '');
    ...
    return (
        <View
          ...
          <Text
              style={[
                styles.replyText,
                {color: isOwnMessage ? 'rgba(255,255,255,0.8)' : theme.text},
              ]}
              numberOfLines={1}>
            {message.replyTo.contentType === 'text'
                ? message.replyTo.content
                : `${
                    message.replyTo.contentType.charAt(0).toUpperCase() +
                    message.replyTo.contentType.slice(1)
                }`}
          </Text>
        </View>
    );
  ...
  const handleContentPress = () => {
    ...
    if (message.contentType === 'image') {
      ...
      Linking.openURL(mediaUrl).catch(err => {
        console.error('Error opening image URL:', err);
        Alert.alert('Error', 'Could not open the image');
      });
    } else if (message.contentType === 'video') {
      console.log('Downloading video for playback');
      // For videos, we'll download and play locally instead of opening in browser
      const fileName = message.mediaName || `video_${Date.now()}.mp4`;
      downloadFile(mediaUrl, fileName);
    } else if (message.contentType === 'document') {
        ...
        <Animated.View
          ...
          <TouchableOpacity
              ...
              onPress={() => {
                if (onPress) {
                  onPress(message);
                } else if (message.contentType !== 'text' && message.contentType !== 'audio') {
                  handleContentPress();
                }
              }}
              style={{ width: '100%' }}>
...
const styles = StyleSheet.create({
  ...
  documentContainer: {
    ...
    padding: 8,
    borderRadius: 8,
    width: 240, // Set a fixed width for document messages
  },
  ...
  selectedMessage: {
    borderWidth: 2,
    borderColor: '#2196F3', // Highlight color
    opacity: 0.9,
  },
  // New styles for swipe-to-reply
  replyIconContainer: {
    position: 'absolute',
    top: '50%',
    marginTop: -15, // Half of the icon height
    zIndex: 0,
  },
  replyIcon: {
    width: 30,
